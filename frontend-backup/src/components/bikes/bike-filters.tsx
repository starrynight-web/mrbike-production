...
export function BikeFilters({ brands, totalCount }: BikeFiltersProps) {
    ...
    const updateUrl = useCallback(() => {
        ...
        router.push(`/bikes${queryString ? `?${queryString}` : ""}`, { scroll: false });
                ...
                <div className="space-y-2">
                    {PRICE_RANGES.map((range) => {
                        const isSelected =
                            ...
                            (range.max === Infinity ? priceRange[1] >= 10000000 : priceRange[1] === range.max);
                        return (
                            <button
                                key={range.label}
            ...
            <div>
                ...
                <div className="space-y-2">
                    {CC_RANGES.map((range) => (
                        <button
                            key={range.label}
                            onClick={() => setCcRange([range.min, range.max === Infinity ? 1000 : range.max])}
    ...
    return (
        <div className="flex items-center justify-between gap-4 mb-6">
            ...
            <p className="text-sm text-muted-foreground">
                <span className="font-medium text-foreground">{totalCount}</span> bikes found
            </p>

            <div className="flex items-center gap-2">
                {/* Sort Dropdown */}
                <Select value={sortBy} onValueChange={(v) => { setSortBy(v); updateUrl(); }}>
                    ...
                    <div>
                        ...
                        <div className="space-y-1">
                            {PRICE_RANGES.map((range) => (
                                <button
                                    key={range.label}
                                    onClick={() => setPriceRange([range.min, range.max === Infinity ? 10000000 : range.max])}
