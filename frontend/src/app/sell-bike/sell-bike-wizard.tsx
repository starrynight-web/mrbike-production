...
import { useCreateUsedBike } from "@/hooks/use-used-bikes";
...
export function SellBikeWizard() {
    ...
    const { mutateAsync: createListing } = useCreateUsedBike();
    ...
    const handleLogin = () => {
        signIn(undefined, { callbackUrl: "/sell-bike" });
    };
    ...
    const onSubmit: SubmitHandler<FormData> = async (data) => {
        ...
        try {
            ...
            formData.append("title", `${data.brand} ${data.model} ${data.year}`);
            formData.append("price", String(data.price));
            formData.append("mileage", String(data.kmDriven));
            formData.append("manufacturing_year", String(data.year));
            formData.append("condition", data.condition);
            formData.append("description", data.description);
            formData.append("location", data.location);

            // Always use custom_brand and custom_model for simplicity
            // The backend allows these fields when bike_model is not specified
            formData.append("custom_brand", data.brand);
            formData.append("custom_model", data.model);
            ...
            // Append accident history field (stringified boolean or details)
            formData.append('accident_history', String(data.accidentHistory));

            await createListing(formData);

            toast.success("Ad posted successfully! It is under review.");
            router.push("/profile"); // Redirect to profile/listings
        } catch (error: any) {
            console.error("Sell bike error:", error);
            toast.error(error?.message || error?.error?.message || "Failed to post ad. Please try again.");
        } finally {
            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <div className="flex gap-2 mt-2 overflow-x-auto pb-2">
                        {images.map((img, i) => (
                            <img key={i} src={img.preview} className="h-20 w-28 object-cover rounded border" alt="" />
                        ))}
                    </div>
                </div>
