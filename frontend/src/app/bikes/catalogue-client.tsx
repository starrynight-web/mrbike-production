...
import { BikeCard, BikeFilters, BikeFiltersSidebar, CompareBar } from "@/components/bikes";
import { Skeleton } from "@/components/ui/skeleton";
import { useBikes, useBrands } from "@/hooks/use-bikes";
import { useFilterStore } from "@/store";
import type { Bike, BikeFilters as BikeFiltersType } from "@/types";

export function BikeCataloguePage() {
    const searchParams = useSearchParams();
    const { selectedBrands, selectedCategories, priceRange, sortBy } = useFilterStore();

    // Build filters from URL and store
    const filters: BikeFiltersType = {
        brand: selectedBrands.length > 0 ? selectedBrands : undefined,
        category: selectedCategories.length > 0 ? selectedCategories as any : undefined,
    ...
    // Fetch data
    const { data: bikesData, isLoading: bikesLoading, error: bikesError } = useBikes(filters);
            ...
            <div className="container py-6 md:py-8">
                <div className="flex gap-8">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {/* Pagination */}
                        {!bikesLoading && bikesData?.meta && bikesData.meta.totalPages > 1 && (
                            <div className="mt-10 flex justify-center">
                                <nav className="flex items-center gap-2">
                                    {Array.from({ length: Math.min(bikesData.meta.totalPages, 5) }).map(
                                        (_, i) => {
                                            const page = i + 1;
