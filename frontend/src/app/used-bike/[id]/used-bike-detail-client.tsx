...
import { useUsedBikesNearBudget } from "@/hooks/use-bikes";
        ...
        return (
            <div className="container py-20 text-center">
                ...
                <p className="text-muted-foreground mb-8">
                    The used bike listing you're looking for doesn't exist or has been removed.
                </p>
                <Button asChild>
            ...
            <div className="bg-muted/50 border-b">
                <div className="container py-3">
                    <nav className="flex items-center gap-2 text-sm text-muted-foreground whitespace-nowrap overflow-x-auto">
                        ...
                        <Link href="/used-bikes" className="hover:text-foreground">Used Bikes</Link>
                        <ChevronRight className="h-4 w-4 shrink-0" />
                        <span className="text-foreground font-medium truncate">{bike.bikeName}</span>
                    </nav>
                </div>
            </div>
                ...
                <div className="grid lg:grid-cols-[1fr_380px] gap-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                            <div className="relative aspect-[4/3] md:aspect-video bg-black rounded-xl overflow-hidden group">
                                ...
                                {bike.images.length > 1 && (
                                    <>
                                        <button
                                            onClick={() => setActiveImageIndex(prev => prev === 0 ? bike.images.length - 1 : prev - 1)}
                                        ...
                                        <button
                                            onClick={() => setActiveImageIndex(prev => prev === bike.images.length - 1 ? 0 : prev + 1)}
                            ...
                            {bike.images.length > 1 && (
                                <div className="flex gap-2 overflow-x-auto py-1">
                                    {bike.images.map((img: any, index: number) => (
                                        <button
                                            key={index}
                                            onClick={() => setActiveImageIndex(index)}
                                            className={cn(
                                                "shrink-0 w-20 h-16 rounded-lg overflow-hidden border-2 transition-all relative",
                                                activeImageIndex === index ? "border-primary" : "border-transparent opacity-70 hover:opacity-100"
                                            )}
                                        >
                        ...
                        <div className="lg:hidden space-y-4">
                            <div>
                                ...
                                <div className="flex items-center gap-2 text-muted-foreground text-sm">
                                    <MapPin className="h-4 w-4" /> {bike.location.city} • {formatRelativeTime(bike.createdAt)}
                                </div>
                            </div>
                        ...
                        <Card>
                            ...
                            <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4">
                                ...
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">Model Year</p>
                                    <p className="font-medium">{bike.year}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">Kilometers Run</p>
                                    <p className="font-medium">{bike.kmDriven.toLocaleString()} km</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">Accident History</p>
                                    <p className="font-medium">{bike.accidentHistory ? "Yes" : "No"}</p>
                                </div>
                                <div>
                    ...
                    <div className="space-y-6">
                        ...
                        <Card className="hidden lg:block">
                            <CardContent className="p-6">
                                ...
                                <div className="flex items-center gap-2 text-muted-foreground text-sm mb-4">
                                    <MapPin className="h-4 w-4" /> {bike.location.city} • <Clock className="h-3 w-3" /> {formatRelativeTime(bike.createdAt)}
                                </div>
                                <div className="text-3xl font-bold text-primary mb-6">
                                    {formatPrice(bike.price)}
                                </div>
                                <div className="space-y-3">
                                    <Button size="lg" className="w-full gap-2 text-lg font-bold bg-[#25D366] hover:bg-[#128C7E] text-white">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button variant="ghost" className="flex-1 text-destructive hover:bg-destructive/10">
