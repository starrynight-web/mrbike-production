...
import { PRICE_RANGES, BD_CITIES, BIKE_CONDITIONS } from "@/config/constants";
...
export function UsedBikeFilters() {
    ...
    const toggleBrand = (slug: string) => {
        ...
        if (brands.includes(slug)) {
            ...
            brands.filter((b) => b !== slug).forEach((b) => newSearchParams.append("brand", b));
        } else {
    ...
    const toggleCondition = (value: string) => {
        ...
        if (conditions.includes(value)) {
            ...
            conditions.filter((c) => c !== value).forEach((c) => newSearchParams.append("condition", c));
        } else {
    ...
    const FilterContent = ({ mobile = false }: { mobile?: boolean }) => (
        <div className={cn("space-y-6", mobile ? "px-1" : "")}>
            ...
            <div className="flex items-center justify-between">
                <h3 className="font-semibold text-lg">Filters {activeFilterCount > 0 && `(${activeFilterCount})`}</h3>
                {activeFilterCount > 0 && (
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between text-sm">
                    <span className="border px-2 py-1 rounded bg-muted">{formatPrice(minPrice)}</span>
                    <span className="text-muted-foreground">-</span>
                    <span className="border px-2 py-1 rounded bg-muted">{formatPrice(maxPrice)}</span>
                </div>
            </div>
            ...
            {/* Dynamic Sections */}
            <Accordion type="multiple" defaultValue={["condition", "brand"]} className="w-full">
                {/* Condition */}
                        ...
                        <div className="space-y-2 pt-1 max-h-[300px] overflow-y-auto pr-2">
                            {brands?.map((brand: Brand) => (
                                <div key={brand.id} className="flex items-center space-x-2">
                                    ...
                                    <span className="text-xs text-muted-foreground">({brand.bikeCount})</span>
                                </div>
                            ))}
                        </div>
                    </AccordionContent>
